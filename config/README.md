...
Runtime configuration for the `ups-battery-shutdown` service.

---

### `ups-battery-shutdown.conf`

Generated by `scripts/ups-service.sh` during setup. Symlinked to `/etc/ups-battery-shutdown.conf` so the installed daemon can find it.
