...
#
# Usage: sudo ./control-node-shutdown.sh [command]
#
...
do_set_threshold() {
    ...
    [[ ! -f "$CONF" ]] && err "Config not found: $CONF — run setup first"
    sed -i "s/^THRESHOLD=.*/THRESHOLD=${N}/" "$CONF"
...
run_setup() {
    ...
    if [[ -f "$CONF" ]]; then
        ...
        echo
        grep -v '^#' "$CONF" | grep -v '^[[:space:]]*$' | sed 's/^/    /'
        echo
...
# Generated by control-node-shutdown.sh on $(date)
